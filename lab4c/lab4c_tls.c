...
int main (int argc, char **argv)
{
    ...
    new_content = ssl_init();
    ssl_client = attach_ssl_to_socket(sockfd, new_content);
...
SSL_CTX* ssl_init(void)
{
    ...
    if (newContent == NULL)
    {
        fprintf(stderr, "error when getting new SSL content\n");
        exit(2);
    }
    return newContent;
}
...
SSL* attach_ssl_to_socket (int socket, SSL_CTX *context)
{
    ...
    if (sslClient == NULL)
    {
        fprintf(stderr, "creation of a new ssl structure failed\n");
        exit(2);
    }
    if (SSL_set_fd(sslClient, socket) < 0)
    {
        fprintf(stderr, "error when set the socketfd for ssl\n");
        exit(2);
    }
    if (SSL_connect(sslClient) != 1)
    {
        fprintf(stderr, "ssl server connection rejected\n");
        exit(2);
    }
    return sslClient;
}
